  ...
  return (
    <div className="flex flex-col h-full">
      <button 
        ...
        className="mb-4 w-full py-3 bg-gradient-to-r from-green-500 to-blue-600 text-white rounded-xl font-bold shadow-lg transform active:scale-95 transition-all text-sm"
      >
        {showAdd ? 'キャンセル' : '+ 友達追加'}
      </button>
      ...
      <div className="flex-grow space-y-3">
        {contacts.length === 0 ? (
          <div className="text-center text-gray-400 mt-10">連絡先はまだありません</div>
        ) : (
...
function ChatRoom({ contact, onBack, getMessages, sendMessage }) {
  ...
  const handleSend = async (e) => {
    e.preventDefault()
    if (text.trim()) {
      await sendMessage(contact.publicKey, text)
      setText('')
      refreshMessages()
    }
  }
...
function App() {
  ...
  const { profile, contacts, loading, addContact, updateProfileName, getChatMessages, sendChatMessage } = useMessenger()
  ...
  if (!profile.name) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 bg-white animate-fade-in text-center">
        <h2 className="text-2xl font-bold mb-2">👋 Welcome to Wave</h2>
        <input 
          type="text" 
          ...
          <div className="p-4 space-y-6">
            <div className="bg-gradient-to-br from-white to-gray-50 p-6 rounded-3xl shadow-sm border border-white">
              ...
              <div className="space-y-4">
                ...
                <button className="w-full py-3 bg-gray-100 text-gray-600 rounded-xl text-xs font-bold hover:bg-gray-200 transition-colors">IDをコピー</button>
              </div>
            </div>

            <section>
              <h3 className="text-xs font-bold text-gray-400 mb-3 ml-2 uppercase tracking-widest">Recent Activity</h3>
              <div className="space-y-3">
                {contacts.slice(0, 3).map(c => (
                  <div key={c.publicKey} onClick={() => { setSelectedContact(c); setActiveTab('chats'); }} className="bg-white p-4 rounded-2xl flex items-center shadow-sm cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    <div className="ml-3"><div className="text-sm font-bold">{c.name}</div></div>
                  </div>
                ))}
        ...
        {activeTab === 'settings' && (
          <div className="p-4">
             <div className="bg-white p-6 rounded-3xl shadow-sm">
                <h3 className="font-bold border-b pb-2 mb-4">環境設定</h3>
                <button onClick={() => updateProfileName('')} className="text-red-500 text-sm font-bold w-full text-left py-2">ログアウト</button>
             </div>
          </div>
        )}
      </main>
